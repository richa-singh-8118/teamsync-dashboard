  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '2rem', background: 'var(--bg-main)' }}>
      <div className="card" style={{ padding: '3rem', width: '100%', maxWidth: '480px', textAlign: 'center' }}>
        ...
        <h2 style={{ fontSize: '1.5rem', marginBottom: '0.5rem', color: 'var(--text-dark)' }}>Welcome back</h2>
        <p style={{ color: 'var(--text-gray)', marginBottom: '2rem', fontSize: '0.95rem', fontWeight: 500 }}>Please enter your details to sign in.</p>
        ...
        <form onSubmit={handleSubmit} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '1.25rem', textAlign: 'left' }}>
          ...
          <button type="submit" className="btn btn-primary" style={{ marginTop: '1rem', width: '100%', padding: '1rem', fontSize: '1rem' }}>Sign In</button>
        </form>
        
        <p style={{ marginTop: '2.5rem', color: 'var(--text-gray)', fontSize: '0.9rem', fontWeight: 500 }}>
          Don't have an account? <Link to="/signup" style={{ fontWeight: 700, color: 'var(--accent-blue)' }}>Sign up</Link>
        </p>
      </div>
    </div>
  );
